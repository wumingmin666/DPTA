...
class YOLOWorldHead(YOLOv8Head):
    ...
    def predict(self,
        ...
        #修改了一下
        # outs = self(img_feats, txt_feats, txt_masks)
        outs = self(img_feats, txt_feats, txt_masks)[:2]
        predictions = self.predict_by_feat(*outs,
                                           batch_img_metas=batch_img_metas,
                                           rescale=rescale)
        return predictions
    ...
    def loss_by_feat(
        ...
        """
        # print("===================")
        # print(bbox_preds[0].size())#torch.Size([2, 4, 80, 80])  3是特征图的level
        ...
        flatten_pred_bboxes = [
            bbox_pred.permute(0, 2, 3, 1).reshape(num_imgs, -1, 4)           #level,num_image,n,4
    ...
    def predict_by_feat(self,
        ...
        cfg.multi_label = multi_label#True
        ...
        flatten_bbox_preds = torch.cat(flatten_bbox_preds, dim=1)#目前还是偏移量
