...
class TextPrompt(BaseModule):
    def __init__(self,
                 length: int,
                 embed_dim: int,
                 embedding_init: str = 'xavier_uniform_',
                 init_cfg: OptMultiConfig = None) -> None:
        super().__init__(init_cfg=init_cfg)
        self.length = length
        self.prompt_embeddings = nn.Parameter(torch.zeros(1, length, embed_dim))
        if embedding_init == 'xavier_uniform_':
            nn.init.xavier_uniform_(self.prompt_embeddings.data)
        else:
            nn.init.normal_(self.prompt_embeddings.data, mean=0.0, std=0.02)

    def forward(self, batch_size: int) -> Tensor:
        return self.prompt_embeddings.expand(batch_size, -1, -1)

@MODELS.register_module()
class HuggingCLIPLanguageBackboneWithPrompt(BaseModule):
    def __init__(self,
                 model_name: str,
                 prompt_cfg: ConfigType = None,
        ...
        if prompt_cfg is not None:
            self.text_prompt = MODELS.build(prompt_cfg)
            self.token_embedding = self.model.text_model.embeddings.token_embedding
        else:
    ...
    def forward(self, text: List[List[str]]) -> Tensor:
        ...
        if self.text_prompt is not None:
            # manual forward pass
            token_embeds = self.token_embedding(text.input_ids)
            prompt_embeds = self.text_prompt(token_embeds.shape[0])
            inputs_embeds = torch.cat([prompt_embeds, token_embeds], dim=1)

            # add position embeddings
            position_ids = torch.arange(
                inputs_embeds.size(1),
                device=inputs_embeds.device).expand((inputs_embeds.size(0), -1))
            ...
            prompt_mask = torch.ones(
                (text.attention_mask.shape[0], self.text_prompt.length),
                device=text.attention_mask.device,
                dtype=text.attention_mask.dtype)
            new_attention_mask = torch.cat(
                [prompt_mask, text.attention_mask], dim=1)

            # expand attention_mask
            bsz, seq_len = new_attention_mask.size()
            expanded_mask = new_attention_mask[:, None, None, :].expand(
                bsz, 1, seq_len, seq_len).to(final_embeddings.dtype)
            inverted_mask = 1.0 - expanded_mask
            four_d_attention_mask = inverted_mask.masked_fill(
                inverted_mask.to(torch.bool),
                torch.finfo(final_embeddings.dtype).min)

            encoder_outputs = self.model.text_model.encoder(
                final_embeddings, attention_mask=four_d_attention_mask)
            last_hidden_state = encoder_outputs[0]
            pooled_output = last_hidden_state[
                torch.arange(last_hidden_state.shape[0]),
                text.input_ids.argmax(dim=-1)]
            pooled_output = self.model.text_model.final_layer_norm(
                pooled_output)
            txt_feats = self.model.text_projection(pooled_output)
        else:
