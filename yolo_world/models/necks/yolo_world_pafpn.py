...
@MODELS.register_module()
class PromptedYOLOWorldDualPAFPN(YOLOWorldDualPAFPN):
    """
    A YOLO World Dual PAFPN with learnable text prompts for few-shot tuning.
    """
    def __init__(self,
                 *args,
        ...
        from ..layers.prompts import TextPrompt
        self.text_prompt = TextPrompt(prompt_length, self.guide_channels)

    def forward(self, img_feats: List[Tensor], txt_feats: Tensor) -> tuple:
        """Forward function with text prompt injection."""
        txt_feats = self.text_prompt(txt_feats)
        return super().forward(img_feats, txt_feats)