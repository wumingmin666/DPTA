...
@OPTIM_WRAPPER_CONSTRUCTORS.register_module()
class PromptOptimizerConstructor(YOLOWv5OptimizerConstructor):
    """
    Optimizer constructor for prompt tuning.
    This constructor freezes all parameters of the model except for the
    parameters of the prompt modules.
    """
    ...
    def __call__(self, model: nn.Module) -> OptimWrapper:
        ...
        # This relies on the names we gave our prompt modules
        for name, param in model.named_parameters():
            if 'text_prompt' in name or 'vision_prompt_adapters' in name:
                param.requires_grad = True
                print("============================================================")
                print_log(f'Unfreezing parameter for tuning: {name}', 'current')
